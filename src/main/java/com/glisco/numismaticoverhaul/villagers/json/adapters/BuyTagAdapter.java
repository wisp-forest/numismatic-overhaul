...
public class BuyTagAdapter extends TradeJsonAdapter {
    ...
    public TradeOffers.Factory deserialize(JsonObject json) {
        ...
        final var tag = Identifier.of(JsonHelper.getString(buyObject, "tag"));
    ...
    private static class Factory implements TradeOffers.Factory {
        ...
        public TradeOffer create(Entity entity, Random random) {
            ...
            randomPrice += (int) (randomPrice * 0.10f * MathHelper.nextFloat(random, 0.4f, 2.0f));

            return new TradeOffer(new TradedItem(entries.get(random.nextInt(entries.size())).value(), this.count), CurrencyHelper.getClosest(randomPrice), this.maxUses, this.experience, multiplier);
        }
    }
